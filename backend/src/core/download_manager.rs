...
use crate::storage::{
  chunk_id_map::ChunkIdMap,
  file_store::{FileStoreManager, StorageVolumeId}
};
...
impl DownloadManager {
  ...
  pub async fn read_chunk(
    ...
    // Read chunk
    let chunk = self.file_store.read_chunk(volume_id, handle, owner_id, chunk_id, chunk_id_map).await?;
