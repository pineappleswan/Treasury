...
use axum::{extract::{Multipart, State, Path}, response::IntoResponse, Json};
...
use crate::{
  ...
  util::{
    generators::generate_file_handle,
    multipart::*
  },
  AppState
};
...
pub async fn finalise_upload_api(
  ...
  // Length times 2 for key and value and 4 for 4 bytes per value (TODO: remove this value as its only for the debug print)
  let upload_chunk_id_map_size = active_upload.chunk_id_map.len() * 2 * 4;
  
  // Ensure the correct number of bytes have been written to the upload file.
  if active_upload.written_bytes != upload_file_size {
    let bytes_left_to_write = upload_file_size as i64 - active_upload.written_bytes as i64;

    warn!(
      "Couldn't finalise upload by user {}. Bytes left to write: {}.",
      session_data.user_id,
      bytes_left_to_write,
    );

    // TODO: delete this upload

    return (
      StatusCode::BAD_REQUEST,
      format!("Failed to finalise. Closing upload. Bytes left to write: {}", bytes_left_to_write)
    ).into_response();
  }

  // Serialise the chunk id map in the active upload if it isn't empty.
  let chunk_id_map_blob = if active_upload.chunk_id_map.is_empty() {
    None
  } else {
    match active_upload.chunk_id_map.serialise() {
      Ok(blob) => Some(blob),
      Err(err) => {
        error!(
          "Failed to finalise upload from user {} because chunk id map failed to serialise! Handle: {}. Error: {}",
          session_data.user_id,
          path_params.handle,
          err
        );
  ...
  let new_file = UserFileEntry {
    ...
    chunk_id_map: chunk_id_map_blob,
    encrypted_crypt_key: Some(encrypted_crypt_key),
    encrypted_metadata
  };

  debug!("Finalised with chunk id map size of: {}", upload_chunk_id_map_size);
...
pub async fn upload_chunk_api(
  ...
  // Convert to u32 as that is the type used for chunk ids
  ...
  // Add chunk to buffer
  match active_upload.write_chunk(chunk_id, data, &state.file_store).await {
