...
type QRCodePopupSettings = {
	createPopup?: (data: string) => void;
};

type QRCodePopupProps = {
	settings: QRCodePopupSettings
};
...
function QRCodePopup(props: QRCodePopupProps) {
	...
	props.settings.createPopup = async (data: string) => {
		setVisible(true);
	...
	return (
		<div
			class={`absolute flex shrink-0 justify-center items-center self-center backdrop-blur-[2px] w-[100%] h-[100%] z-10 backdrop-brightness-90`}
